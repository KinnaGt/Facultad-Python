...
# ‘https’.
# Recordar:
# * split(): ¿qué caracter utilizarían para separar? el espacio 
# * string in frase: para saber si un substring está incluido dentro de una frase
# * ¿podemos preguntar si un string se encuentra en una
...
# Printear solo con for
arr_words = texto.split(' ')
for word in arr_words:
    if ('http' in word):
        print(word)


# Podemos printear con filter tmb con esta forma
str_match = list(filter(lambda x: ('http' in x) , arr_words))
print(str_match)

print('-'*150) # Linea para acomodar terminal


# O con expresiones regulares
import re
pattern=re.compile(r'http') 
str_match = [x for x in arr_words if re.search('http', x)]
print(str_match)


# Para encontrar un string en una lista sin recorrer podemos usar el metodo
if('http' in texto):
    print('Existe')